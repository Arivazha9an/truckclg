...
import 'package:truckclgproject/BharathBenz/screens/updateforms/updateservice.dart';
import 'package:truckclgproject/constants/colors.dart';
import 'package:truckclgproject/widgets/customappbar.dart'; 

class Serviceretrieve extends StatefulWidget {
  const Serviceretrieve({super.key});
  @override
  _ServiceretrieveState createState() => _ServiceretrieveState();
}

class _ServiceretrieveState extends State<Serviceretrieve> {
  ...
  Future<File> _generatePdf(List<DocumentSnapshot> documents) async {
            ...
            return pw.Container(
              ...
              child: pw.Column(
                ...
                children: [
                  ...
                  pw.Text('PKm Riding: ${data['KM Riding'] ?? ''}'),
                  pw.Divider(),
                ],
              ),
            );
    ...
    final path =
        '${directory.path}/service_data_${DateTime.now().toString().replaceAll(':', '-')}.pdf';
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: const CustomAppBar(title: 'Service Data', isGoBack: true),
      body: Column(
        children: [
          Padding(
