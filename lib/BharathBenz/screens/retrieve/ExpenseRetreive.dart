...
import 'package:truckclgproject/BharathBenz/screens/updateforms/expenseupdate.dart';
import 'package:truckclgproject/constants/colors.dart';
import 'package:truckclgproject/widgets/customappbar.dart'; 

class Expenseretreieve extends StatefulWidget {
  const Expenseretreieve({super.key});

  @override
  State<Expenseretreieve> createState() => _ExpenseretreieveState();
}

class _ExpenseretreieveState extends State<Expenseretreieve> {
 TextEditingController _startDateController = TextEditingController();
  ...
  Future<File> _generatePdf(List<DocumentSnapshot> documents) async {
          ...
          itemBuilder: (context, index) {
            ...
            return pw.Container(
              ...
              child: pw.Column(
                ...
                children: [
                  pw.Text('ExpenseType: ${data['ExpenseType'] ?? ''}'),
                  pw.Text('Date: ${data['Date'] ?? ''}'),
                  pw.Text('Amount: ${data['Amount'] ?? ''}'),
                  pw.Text('Km: ${data['Km'] ?? ''}'), 
                  pw.Text('VehicleNo:${data['vehiclenumber'] ?? ''}')  ,           
                  pw.Divider(),
                ],
              ),
            );
    ...
    final path =
        '${directory.path}/expense_data_${DateTime.now().toString().replaceAll(':', '-')}.pdf';
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: const CustomAppBar(title: 'Expense Data', isGoBack: true),
      body: Column(
        children: [
          Padding(
