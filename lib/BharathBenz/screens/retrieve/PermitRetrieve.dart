import 'dart:io';
import 'package:truckclgproject/BharathBenz/screens/updateforms/updatepermite.dart';
...
class Permitretrieve extends StatefulWidget {
  const Permitretrieve({super.key});

  @override
  State<Permitretrieve> createState() => _PermitretrieveState();
}

class _PermitretrieveState extends State<Permitretrieve> {
            ...
            return pw.Container(
              ...
              child: pw.Column(
                ...
                children: [
                  ...
                  pw.Text('Permit Type: ${data['Permit Type'] ?? ''}'),
                  pw.Text('permit Number: ${data['Permit Number'] ?? ''}'),
                  pw.Text('Expiry: ${data['Expiry'] ?? ''}'),
                  pw.Text('Issue: ${data['Issue'] ?? ''}'),
                  pw.Text('Permit card: ${data['Permit Card'] ?? ''}'),
                  pw.Divider(),
                ],
              ),
            );
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: const CustomAppBar(title: 'Permit Data', isGoBack: true),
      body: Column(
        children: [
          Padding(
                  ...
                  return ListView.builder(
                    ...
                    itemBuilder: (context, i) {
                      ...
                      String date = data['date'] ?? '';
                       String permitType = data['Permit Type'] ?? '';
                      String permitNo = data['Permit Number'] ?? '';
                      String expiry = data['Expiry'] ?? '';
                      String issue = data['Issue'] ?? '';
                      String permitCard = data['Permit Card'] ?? '';
                                    ...
                                    IconButton(
                                      onPressed: () {
                                        Navigator.push(
                                          context,
                                          MaterialPageRoute(
                                            builder: (context) => UpdateFormPermit(
                                              docId: docId,
                                              data: data,
                                            ),
                                          ),
